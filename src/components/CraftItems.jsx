      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
        {
          userCrafts.slice(0, 6).map(userCraft =>
            ...
            rounded-lg shadow-md bg-gray-50 text-violet-600 mx-auto"
            > 
              {/* Craft Owner */}
              ...
              {/* card center */}
              <div>
                <img src={userCraft.photo || "photo not found"} alt="" className="object-cover w-full mb-4 h-60 sm:h-96 bg-gray-500" />
                <h2 className="mb-1 text-xl font-semibold">{userCraft.itemName}</h2>
                <p className="text-sm text-gray-600">{userCraft.description}</p>
              </div>
              {/* card bottom */}
